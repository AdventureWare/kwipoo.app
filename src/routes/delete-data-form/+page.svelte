<script lang="ts">
  ...
  const pageTitle = "Delete Data Request | Kwipoo";
  const pageDescription =
    "Request deletion of your Kwipoo account and associated data through the public deletion-request page.";

  const requestChecklist = [
    "Use the email address tied to your Kwipoo account.",
    "Add any extra context that helps identify the account quickly.",
    "After submitting, send the prefilled email if your mail app opens.",
  ] as const;

  const nextSteps = [
    "We review deletion requests through the support mailbox.",
    "We may need follow-up information if we cannot confidently identify the account.",
    "You can use the support or privacy pages if you need broader help first.",
  ] as const;
    ...
    "@graph": [
      {
        ...
        url: toAbsoluteMarketingUrl("/delete-data-form"),
        isPartOf: {
          "@type": "WebSite",
          name: SITE_NAME,
      ...
      getBreadcrumbJsonLd([
        { name: "Home", path: "/" },
        { name: "Delete Data Request", path: "/delete-data-form" },
      ]),
    ],
  };
...
<svelte:head>
  ...
  <meta property="og:url" content={toAbsoluteMarketingUrl("/delete-data-form")} />
...
<div class="grid gap-10 pb-12">
  <header class="grid gap-5">
    ...
    >
      ...
      <span class="text-brand-body">Delete Data Request</span>
    </nav>
    ...
    >
      <div class="space-y-4">
        ...
        >
          Request deletion of your Kwipoo account and associated data.
        </h1>
        <p class="max-w-3xl text-lg leading-8 text-brand-body">
          Use this page to prepare a deletion request for the email address tied to
          your Kwipoo account. After you submit the form, you can send a prefilled
          request through your email app if it opens on your device.
        </p>
      </div>
      ...
      >
        <div class="space-y-3">
          ...
          <h2 class="text-[1.4rem] font-semibold leading-tight text-surface-950">
            This page gives users a live deletion-request path outside the app.
          </h2>
      ...
      {#if form?.success}
        ...
        >
          <h3 class="text-[1.15rem] font-semibold text-surface-950">
            Your deletion request is ready to send.
          </h3>
      ...
      <form method="POST" class="mt-6 grid gap-5">
        ...
        <label class="grid gap-2">
          ...
          <select
            ...
            name="platform"
          >
            <option value="unknown" selected={(form?.values?.platform ?? "unknown") === "unknown"}>
              Not sure
            </option>
            <option value="ios" selected={form?.values?.platform === "ios"}>iOS</option>
            <option value="android" selected={form?.values?.platform === "android"}>
              Android
            </option>
            <option value="web" selected={form?.values?.platform === "web"}>Web</option>
          </select>
          {#if form?.errors?.platform}
            <span class="text-sm font-medium text-red-700">{form.errors.platform}</span>
          {/if}
        </label>
      ...
      >
        ...
        <div class="mt-5 rounded-[1.1rem] border border-surface-200 bg-white p-4">
          <h3 class="text-[1.08rem] font-semibold text-surface-950">
            Need another support path first?
          </h3>
          <p class="mt-2 text-[0.96rem] leading-7 text-brand-body">
            If your question is broader than deletion alone, the support and legal
            pages stay available alongside this form.
          </p>
