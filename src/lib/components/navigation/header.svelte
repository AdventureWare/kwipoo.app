    ...
    <div class="flex justify-between items-center h-16">
      ...
      <div class="flex-shrink-0">
        <a href="/" class="flex items-center">
          <span class="text-2xl font-bold text-primary">Kwipoo</span>
        </a>
      </div>
      ...
      <div class="hidden md:flex items-center space-x-8">
        {#each navItems as item}
          <a
            href={item.href}
            class="px-3 py-1 rounded-md text-gray-700 hover:text-primary transition-colors duration-200 font-medium {page
      ...
      >
        <div class="px-2 pt-2 pb-3 space-y-1">
          {#each navItems as item}
            <a
              href={item.href}
              class="block px-3 py-2 text-gray-700 hover:text-primary hover:bg-gray-50 rounded-md font-medium transition-colors duration-200 {page
                .url.pathname === item.href
                ? 'bg-primary/10'
                : ''}"
