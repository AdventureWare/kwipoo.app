...
5. If the user-facing workflow changed, update the relevant narrative page in `website/src/lib/content/docs/pages.ts`.

### When feature behavior changes but fields do not

1. Update the relevant page copy in `website/src/lib/content/docs/pages.ts`.
...
- `website/src/lib/content/docs/entity-reference.generated.ts`
- `website/src/lib/content/docs/pages.ts`

## Practical Rule

If a change touches a core entity field, form, or the meaning of a feature, assume the docs need review in the same PR or branch. If the table is wrong, fix the app reference. If the explanation is wrong, fix the website page. If both changed, update both.
